...
std::string path("database/database.json");
...
auto findCode = [&](const std::string& sCode){
     ...
     if( it != categoryValues.end()){
        return json[it->second][0][sCode];
     }

     FAIL("The code was not found !");
};
...
TEST_CASE("Getting information about a specific code","[Specific code test]"){
    ...
    auto foundCode = findCode(enteredCode);
